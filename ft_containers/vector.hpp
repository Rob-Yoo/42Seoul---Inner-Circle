...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/13 16:10:29 by jinyoo            #+#    #+#             */
/*   Updated: 2023/01/28 18:11:02 by jinyoo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
public:
	// constructor
	explicit vector(): _alloc(allocator_type()), _container(0), _capacity(0), _size(0) {}
	...
	vector(InputIterator first, InputIterator last,  const allocator_type& alloc = allocator_type(), typename ft::enable_if<!is_integral<InputIterator>::value, InputIterator>::type = 0) // 바로 위의 생성자와 컴파일 단계에서 컴파일러가 헷갈릴 수 있음
		: _alloc(alloc), _container(0), _capacity(0), _size(0) {
	...
	void resize(size_type n, value_type val = value_type()) { // capacity 뿐만 아니라 size도 나머지 공간을 두 번쨰 인자를 채워서 늘림
		if (n > _capacity) { // 처음 capacity보다 큰 capacity로 늘리면 현재 capacity의 2배 만큼 늘려놓음
			if (n < 2 * _capacity)
				reserve(2 * _capacity);
			else
				reserve(n);
	...
	void reserve(size_type n) { // resize와 달리 단순히 용량만 늘려줌 size나 element들에 관여 x
		if (n > max_size())
			throw std::length_error("Error: Cannot reserve capacity more than the max_size");
		if (n > _capacity) {
