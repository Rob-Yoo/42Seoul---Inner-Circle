...
/*   Updated: 2022/02/03 04:03:20 by jinyoo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	B_to_A(t_all *all, int sign, int B_chunk_size)
{
	...
	{
		if (all->deq_B->size == 3)
			ps_three_arg_B(all, find_pivot(all->deq_B, 3));
	...
	{
		pivot = find_pivot(all->deq_B, B_chunk_size);
		if_swap(all, 'b', B_chunk_size);
...
void	A_to_B(t_all *all, int sign, int A_chunk_size)
{
	...
	{
		if (all->deq_A->size == 3)
			ps_three_arg_A(all, find_pivot(all->deq_A, 3));
		if (!is_ascending(all->deq_A, A_chunk_size))
			swap('a', all);
	}
	else
	{
		pivot = find_pivot(all->deq_A, A_chunk_size);
		if_swap(all, 'a', A_chunk_size);
		B_chunk_size = make_B_chunk(all, pivot, A_chunk_size, sign);
		if (!B_chunk_size)
			return ;
		new_A_chunk_size = A_chunk_size - B_chunk_size;
		if (sign == SECOND_RECUR)
			A_to_B(all, SECOND_RECUR, new_A_chunk_size); // 두 번째 재귀
		else
			A_to_B(all, FIRST_RECUR, new_A_chunk_size); // 첫 번째 재귀
		if (all->deq_B->size > 0)
